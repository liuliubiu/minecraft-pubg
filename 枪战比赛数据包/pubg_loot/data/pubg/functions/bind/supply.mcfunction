...
execute align xyz positioned ~ ~-1 ~ if block ~ ~ ~ #pubg:containers run data merge block ~ ~ ~ {LootTable:"pubg:chests/supply",CustomName:'{"text":"物资箱","color":"green","italic":false}'}
execute align xyz positioned ~ ~-1 ~ if block ~ ~ ~ #pubg:containers positioned ~0.5 ~0.5 ~0.5 run summon marker ~ ~ ~ {Tags:["pubg_loot","pubg_supply"]}
execute align xyz positioned ~ ~-1 ~ if block ~ ~ ~ #pubg:containers run tellraw @s {"text":"脚下的容器已改成物资箱，下一局会跟着 /function pubg:reset 一起重置。","color":"green"}
