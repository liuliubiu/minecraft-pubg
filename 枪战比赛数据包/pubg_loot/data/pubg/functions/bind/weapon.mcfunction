...
execute align xyz positioned ~ ~-1 ~ if block ~ ~ ~ #pubg:containers run data merge block ~ ~ ~ {LootTable:"pubg:chests/weapon",CustomName:'{"text":"武器箱","color":"red","italic":false}'}
execute align xyz positioned ~ ~-1 ~ if block ~ ~ ~ #pubg:containers positioned ~0.5 ~0.5 ~0.5 run summon marker ~ ~ ~ {Tags:["pubg_loot","pubg_weapon"]}
execute align xyz positioned ~ ~-1 ~ if block ~ ~ ~ #pubg:containers run tellraw @s {"text":"脚下的容器已改成武器箱，下一局会跟着 /function pubg:reset 一起重置。","color":"red"}
